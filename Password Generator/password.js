...
const SYMBOLS_ARRAY = getAsciiArray(33,47).concat(getAsciiArray(58,64)).concat(getAsciiArray(91,96));
...
formID.addEventListener('submit',e=>{
    ...
    const password = randomPassword(charAmount,includeUpper,includeLower,includeNumbers,includeSymbols);
...
function randomPassword(charAmount,includeUpper,includeLower,includeNumbers,includeSymbols){
    let passCode = [];
    if(includeLower){passCode = passCode.concat(LOWER_CASE_ARRAY)};
    if(includeUpper){passCode = passCode.concat(UPPEER_CASE_ARRAY)};
    if(includeNumbers){passCode = passCode.concat(NUMBERS_ARRAY)};
    if(includeSymbols){passCode = passCode.concat(SYMBOLS_ARRAY)};
